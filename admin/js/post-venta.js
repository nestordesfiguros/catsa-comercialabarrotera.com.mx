    ...
    function leerXML(){
        $.ajax({                    
            url: "ajax/post_venta/leeerXML.php",
            type: "POST",
            data: {archivo:
                function(){
                    return $("#archivo").val(); 
    ...
    $("div#myId").dropzone({ 
        url: "ajax/post_venta/upload-files-dropzon.php",
        paramName: "file",        
        addRemoveLinks: true,   
        ...
        removedfile: function(file) {
         ...
         //   console.log('Resultado: '+file);
            ...
            $.ajax({
                url: "ajax/post_venta/eliminar-archivo-dropzone.php", // Reemplaza con la ruta correcta en tu servidor
                type: "POST",
                data: {
    ...
    $("div#complementos").dropzone({ 
        ...
        init: function() {
            ...
            this.on("success", function(file, response) {
                ...
                $.ajax({
                    url: "ajax/post_venta/leerXMLcomplementopago.php",
                    type: "POST",
                    data: {
        ...
        removedfile: function(file) {
            ...
            $.ajax({
                url: "ajax/post_venta/eliminar-archivo-dropzone.php",
                type: "POST",
                data: {
                    filename: filename 
                },
    ...
    /* DROPZONE MAS ARCHVOS */
    $("div#DropzoneMasArchivos").dropzone({ 
        url: "ajax/post_venta/upload-files-dropzon.php",
        paramName: "file",        
        addRemoveLinks: true,   
        createImageThumbnails: true,
        acceptedFiles: ".pdf, .xml, .xlsx, .doc, .docx, .jpg, .jpeg, .png",
        dictRemoveFile: '<i class="fa fa-trash text-danger"></i>Borrar.',           
        success: 
            function (file, response) {           
            //    console.log(response);
                var json = JSON.parse(response);
                var valorInput=$("#archivosMasArchivos").val();
                var valorTodos=valorInput+'|'+json.archivo;
                $("#archivosMasArchivos").val(valorTodos);
            //    console.log('leerXML de dropzone');
                leerXML();
            },
        removedfile: function(file) {
    ...
    function leerXMLcomplementopago(){
        $.ajax({
            url: "ajax/post_venta/leerXMLcomplementopago.php", // ruta en el servidor
            type: "POST",
            data: {
            filename: filename 
            },
    ...
    $(document).ready(function() {  
        ...
        $("#compilarArchivosProcesos").click(
            function(){
            $("#ModalEstatusProceso").modal('toggle');
            }
        );
        
        $("#cerrarMasArchivos").click(
            function(){
                $('#modalMasArchivos').modal('toggle');
        ...
        $("#SiguienteFacturas1").click(
            function(){
                if(!$('#archivo').val()){
                    alertify.alert('Aviso', 'Los archivos pdf y xml son obligatorios', function(){ alertify.success('Aceptar'); });       
                }else{                                                      
                    $("#modalAltaFacturas").modal('show');
        ...
        $("#formAltas").validate(
            ...
            //   console.log('Entro');
                ...
                function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "ajax/post_venta/guarda-factura.php",
                        data: datos,
                        success: function(data)
                        {               
                        console.log(data);                         
        ...
        $('#tablaFacturas').DataTable({                                 
            ajax: 'ajax/post_venta/tabla-facturas.php',
            ordering: true,
            fixedHeader: true,
