            ...
            <div class="col-5">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Usuario y Contraseña de </h3>
                    </div>
                    <div class="card-body">
                        <form action="" id="formpwd" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" id="mail" name="usr" class="form-control" placeholder="Correo electrónico">
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        $("#nombre").on("input", function() {
            const nombreIngresado = $(this).val().trim().toLowerCase();
            let idEncontrado = null;
            let idPuestoEncontrado = null;

            $("#listausuarios option").each(function() {
                const textoOpcion = $(this).val().trim().toLowerCase();
                if (textoOpcion === nombreIngresado) {
                    idEncontrado = $(this).data('id');
                    idPuestoEncontrado = $(this).data('id_puesto');
                }
            });

            $("#id_personal").val(idEncontrado ?? '');
            $("#id_puesto").val(idPuestoEncontrado ?? '');
        });
        ...
        $("#abremodalpwd").click(function() {
            if ($("#mail").val() == '') {
                alertify.alert('Aviso..!', 'Primero tiene que capturar el correo', function() {
                    /* alertify.success('Ok'); */
                });
            } else {
                $.ajax({
            ...
            submitHandler: function(form) {
                ...
                alertify.confirm('Aviso..!', '¿Deseas Guardar los cambios?',
                    function() {
                        $.ajax({
                            type: "POST",
                            url: "ajax/usuarios/guardar-modifica-pwd.php",
                            data: datos,
                            success: function(data) {
                                console.log(data);
                                if (data == 1) {
